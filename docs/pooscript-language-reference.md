...
- **Six guards on `parser.Limits`** (`ScriptLimits`). **Since v1.0 the default is `ScriptLimits.Default` — secure by default:** `MaxDepth = 10`, `MaxParseDepth = 100`, `MaxVariableBytes = 128 MiB` and `RegexTimeout = 1s` are set out of the box (the four *uncatchable-or-uninterruptible* failures — parse-time `StackOverflow`, runtime `StackOverflow`, OOM, and unbounded regex backtracking; `MaxSteps`/`Timeout` stay `null`, since an ordinary script hang is recoverable via the token). A host wanting the pre-1.0 unbounded behaviour sets `parser.Limits = ScriptLimits.None` (a deliberate opt-out); a host wanting different bounds builds `new ScriptLimits { … }`, referencing `ScriptLimits.DefaultMaxDepth`/`DefaultMaxParseDepth`/`DefaultMaxVariableBytes`/`DefaultRegexTimeout` for knobs it keeps. The knobs:
  - `Timeout` (`TimeSpan?`) — a wall-clock deadline, realised as a linked cancellation token so every checkpoint above already honors it for free, on **both** the sync and async paths. On expiry (with no caller cancellation) the host sees `ScriptTimeoutException`; a genuine caller cancel still surfaces as `OperationCanceledException`/`TaskCanceledException` — the two are always distinguishable. The deadline is checked on the **executing thread itself** at every checkpoint (statement, loop iteration, lambda invocation), independent of thread-pool health — except *inside* a single `wait(...)` call, which is interruptible only by cancellation, not by the deadline directly. A script body whose remaining work is a bare `wait(n)` with no checkpoint after it (nothing runs between the wait returning and the script ending) is therefore still bounded only by the thread-pool-scheduled fallback timer, and can run past `Timeout` under a starved pool exactly as before this guard existed — put a statement after the `wait` (even a no-op one) to get a checkpoint back.
  ...
  - `MaxVariables` (`long?`) / `MaxVariableBytes` (`long?`) — bound the script's **own** variable footprint (entry count / approximated bytes); overrun throws `ScriptVariableLimitExceededException`. Growth is caught four ways: at value production/assignment; a periodic, size-proportional sampled walk (catching in-place mutation such as `$list.add($existing)`); an **allocation-denominated growth trigger** that forces a measurement once the process has allocated `MaxVariableBytes` since the last pass, regardless of checkpoint count — the mechanism that keeps a doubling primitive (including a host method that mutates a collection it was handed) from outrunning the sampled walk, bounding retained growth to **≤2× the configured limit** rather than an amount that grows with how fast the primitive happens to double; and a **pre-allocation charge** that charges a collection's requested capacity *before* the backing store is allocated — so `new list(2000000000)`, `list.ensurecapacity(2e9)`, `$list.capacity = n`, and `dict.ensurecapacity(n)` throw a clean exception with **nothing allocated** instead of OOMing the process. **The growth trigger's guarantee differs by target framework:** on net8.0 it reads the runtime's monotonic allocation counter, so the ≤2× bound holds unconditionally; on netstandard2.0 it falls back to the current managed heap size, which a garbage collection can lower, so a script that first inflates the heap with garbage and then grows its live set into the space the collector frees can suppress the trigger — strictly better than no growth trigger at all, but a heuristic rather than a guarantee on that target. A host object graph held by a variable is charged its handle size, not walked into.
    - Full design: `docs/architecture/execution-guards-depth-memory.md`, `docs/architecture/variable-budget-cadence.md`.
- **What the engine still cannot guarantee, even with every guard configured:** a host-supplied method that itself blocks (sync IO, a lock, a raw `Thread.Sleep` in host code); a host-supplied sequence whose single `MoveNext` blocks; a detached `task.run(...)` the script never awaits (its body still observes cancellation and unwinds on its own, but the engine cannot join it); stack overflow from recursion at or beyond `MaxDepth`'s own unwind cost, or from any recursion at all when `MaxDepth` is left unconfigured — see the `MaxDepth` entry above, there is no mechanism watching this beneath the policy ceiling itself. `task.run` bodies get an independent depth budget per task. On **every engine-dispatched invocation** — `$f.invoke(…)` in script, a `task.run` body, and any host extension method that declares a `ScriptContext` parameter and invokes through it — the **whole governing budget** (depth, steps, variables, configured limits and cancellation) is resolved from whichever context is actually invoking the lambda at the moment of the call rather than from wherever that lambda happened to be defined, so a shared helper lambda captured outside a `task.run` body and invoked from inside it is bounded correctly too, the same as one defined inline. ⚠ **This does not extend to `LambdaMethod.Invoke(params object[])`, the overload host C# code calls directly** — that one resolves the whole budget from the context the lambda was *defined* in. **A host extension that accepts a `LambdaMethod` must declare a trailing `ScriptContext` parameter** — the engine injects it, it consumes no script argument, and the script-level call is unchanged (§11.3) — **and invoke through `InvokeFrom(context, …)`.** An extension that calls `Invoke(…)` instead charges every concurrent callback to the defining script's own counters, so *N* simultaneous non-recursive callbacks abort a `MaxDepth` of *N-1* with no recursion involved. `Invoke(…)` is correct only where there is no invoking context at all — host code or a test driving a lambda returned from `Execute`; such a host driving one lambda on several of its own threads hits the same shared-counter shape and must size `MaxDepth` above its own concurrency. **A cached lambda dispatched via `Invoke(…)` also has no deadline at all**, even when `Timeout` is configured: the deadline is realised as a `CancellationTokenSource` that is torn down, un-fired, when the `Execute()` call that produced the lambda returns, so the token a cached lambda holds can never become cancelled — a hostile or merely wedged handler dispatched this way is unbounded in wall-clock time, not merely in steps. **`LambdaMethod.InvokeAsExecution([CancellationToken,] params object[])`** is the fix for host C# dispatching a cached lambda with no execution in progress (Uberkarl's per-frame `onUpdate` shape): it opens a fresh `GuardedExecution` scope for that one dispatch — a **fresh** step budget, a **freshly armed and disposed-in-scope** deadline — while **inheriting** the captured depth and variable budgets by reference (a fresh depth budget per dispatch would read depth 1 forever while the physical stack grows unbounded, turning a catchable `ScriptDepthLimitExceededException` into an uncatchable `StackOverflowException`; a fresh variable budget would stop the memory-guard's walk below the lambda's closure). The host supplies its own `CancellationToken` for the dispatch — the lambda's own captured token, dead or not, never carries over. ⚠ **Never call `InvokeAsExecution` from a host extension reached by a running script** — it grants that dispatch a fresh step budget and a fresh deadline, so a script bouncing through such an extension in a loop would get unbounded work per iteration. Declare a trailing `ScriptContext` parameter and call `InvokeFrom` instead, the same rule as above. The memory guard bounds retained growth and collection pre-sizing (above). The **reflected-method guard** (`MethodGuard`, exposed as `parser.Methods` beside `Types`/`Extensions`; full design `docs/architecture/reflected-method-guard.md`) closes the reflected BCL calls that allocate from a size, format or bulk argument: on the engine's own reachable types, only curated methods dispatch; `PadLeft`/`PadRight`/`new string(char,n)`, `Replace`/`ReplaceLineEndings`/`Split`, and `List.AddRange`/`InsertRange` are charged against the byte budget before anything is built; and `ToString` with a format argument accepts only a standard specifier of at most two precision digits or a custom format of at most 64 characters — in the method form, the `:` operator and interpolation alike, at either arity, on the runtime value of the format. A host-registered type gets the same format policy plus a value ceiling on its own integral arguments (`parser.Methods.Charge`/`Deny` sharpen either). It does **not** bound allocation inside a host-registered type's own method body, it does not apply to the compiled path, and it does not address parse-time recursion or regex backtracking, which are guarded elsewhere. A script may still crash its execution against those residuals — the accepted failure mode: crashing is not escaping. This robustness ceiling is distinct from the escape boundary (above), which is the actual security guarantee; for the highest-risk deployments, running the executor in a killable child process bounds even the crash.
...
- **`)` and `]` are no longer interchangeable closers in indexer and control-statement argument lists** — an intentional, breaking change. `$a[1)` and `if(1]{…}` previously parsed successfully; both now throw `ScriptParserException` (`Mismatched closing token ')' in parameter list, expected ']'.` / `Unexpected token ']' in parameter list, expected ')'.`). Close each list with the bracket that opened it — `[` with `]`, `(` with `)`.
...
- **The reflected-method guard (`parser.Methods`) refuses a few specific shapes as intended breaks**, each with a one-call remedy (`docs/architecture/reflected-method-guard.md` §10.4): a method on a governed type absent from the generated inventory (`parser.Methods.Allow<T>("name")`); `tostring` with a standard-format precision above 99 or a format string above 64 characters (no remedy — this is the vector being closed); `padleft`/`padright`/`new string(char,n)`/`replace`/`replacelineendings`/`split`/`addrange`/`insertrange` whose projected bytes exceed the budget (raise `MaxVariableBytes`, or `ScriptLimits.None`); `split` on a receiver longer than ~4M characters regardless of separator density (raise `MaxVariableBytes`); `addrange`/`insertrange` with a lazy, non-`ICollection` argument (materialise first with `.toarray()`); and a host-registered type's own integral-argument member above the one-byte-per-unit ceiling H2 assumes (`parser.Methods.Charge<T>(...)` with the real per-unit cost, or `Deny<T>(...)`/`Ungovern<T>()`).
