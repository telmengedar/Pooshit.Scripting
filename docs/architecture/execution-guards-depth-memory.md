...
**Narrowed by the reflected-method guard (`docs/architecture/reflected-method-guard.md`, DiVoid #7856/#7872), not removed.** The first bullet above — allocation inside an arbitrary host method body — is the residual that design closes for the *specific, nameable* shapes: `String.PadRight`/`PadLeft`/`.ctor(char,int)`/`Replace`/`ReplaceLineEndings`/`Split` and `List<>.AddRange`/`InsertRange` are charged against the byte budget on the engine's own reachable types (its §8); a `ToString(format)` call is bounded by a format-string policy on **any** `IFormattable` receiver, including a host-injected one (its §9, H1); and a host-injected type's own integral arguments are bounded by a minimum-plausible-cost ceiling unless the host supplies an exact `Charge` row (its §7.6.3, H2). What remains open, unchanged from this section: a host method that allocates heavily from its own logic rather than from a script-controlled size/format/bulk argument (`Report.Render()`) — invisible to every shape rule, closable only by the host's own `Deny<T>()` or by process/container isolation.
