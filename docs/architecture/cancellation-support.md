...
> **Superseded on this point, 2026-08-08.** "That is the entire mechanism" stopped being true: `CancellationTokenSource.CancelAfter`'s callback is a thread-pool work item, so under a starved pool (a `[Parallelizable]` suite full of blocking script executions is enough) the deadline itself arrives late or never in time — measured 510ms/1012ms for a requested 100ms, and a body that finishes between the nominal deadline and the delayed callback returns as if nothing were wrong (DiVoid #7897/#7898). The mechanism is now two-layered: `CancelAfter` remains as a fallback for work that never reaches a checkpoint, and a `DeadlineGuard` (an absolute, `Stopwatch`-based deadline shared by reference across every `ScriptContext` of the execution, the same sharing shape as `StepBudget`/`DepthBudget`/`VariableBudget`) is checked on the **executing thread** at every `ScriptContext.Guard()` call, so enforcement between statements no longer depends on thread-pool health. The consequences list above still holds — zero new checks on the sync path beyond the one `DeadlineGuard.Check()` per checkpoint (measured ~22ns, paid only when `Limits.Timeout.HasValue`), the linked source is still disposed on every exit path, and the caller-cancel/timeout classification below is unchanged. No text elsewhere in this section is rewritten — this note is the pointer.

> **Correction to the correction, same day.** The note above also claimed the `DeadlineGuard` "clamps `Wait.WaitInterruptible`'s chunk to the remaining time" for promptness *during* a single `wait(...)` call. That clamp was implemented and passed 45+ local full-suite TRX-parsed loops, but a second reviewer's independent 50-run loop caught it failing twice (`Timeout_CachedLambdaInvokeAsExecutionThrowsOnEveryDispatchIncludingFirst` and the starvation test itself, both completing normally at ~111ms against a 100ms deadline instead of throwing). Extensive instrumented reproduction (16+ full-suite and processor-constrained runs) did not reproduce the exact failure locally, so the specific mechanism was never confirmed by direct observation — but its safety could not be proven either, and a silent early return from `wait(n)` would corrupt any script relying on `wait` for timing rather than cancellation. The clamp was dropped entirely; `Wait.WaitInterruptible` is back to its pre-#7897 form (interruptible only by `CancellationToken`, no deadline awareness of its own). Enforcement inside a single long `wait(...)` call is therefore back to `CancelAfter` alone — the original #7897 weakness — bounded only *between* statements by `ScriptContext.Guard()`'s `DeadlineGuard.Check()` once the wait itself returns. `Timeout_HoldsUnderStarvedThreadPool` was re-scoped to assert that bound rather than promptness against the configured `Timeout` value, and a new negative test, `Wait_DoesNotReturnEarlyWhenDeadlineNotYetReached`, pins that a `wait(n)` with a far-off deadline runs its full requested duration — so a future reintroduction of a similar clamp fails loudly rather than silently. No text elsewhere in this section is rewritten — this note is the pointer.
...
> **Superseded on this point, 2026-08-08.** T14 as originally written raced `Task.Run`'s own worker-start scheduling: `Task.Run(action, ct)` skips its delegate entirely when `ct` is already cancelled while the work item still sits queued, so a starved pool could cancel before the worker ever entered the `using` — nothing was ever acquired, so `Dispose` was correctly never observed, and the assertion failed on a run that never started rather than on a real unwind-ordering defect (DiVoid #7892/#7898 root-caused this; the engine's unwind ordering itself was cleared). T14 now waits on a `SignalingVariableProvider` that marks the moment the engine resolves `$d`, proving the worker has actually started, before cancelling — the assertion is unchanged. Two more tests join the table on the timeout side of the same starved-pool class: `Timeout_HoldsUnderStarvedThreadPool` (§8.2's superseded note — since amended a second time, the deadline fires but not necessarily *promptly against the configured Timeout*, only bounded once the in-flight `wait(...)` returns) starves the pool the way T9 never needed to; `Timeout_CallerCancelBeforeDeadlineStillThrowsOperationCanceled` pins that the added `DeadlineGuard` self-check still defers to the caller-cancel classification in T15's §9.1 sense. Two more tests were added after §8.2's second correction: `Wait_DoesNotReturnEarlyWhenDeadlineNotYetReached` (negative pin against the reverted `Wait.WaitInterruptible` clamp) and `Invoke_OnCachedLambdaAfterDefinitionTimeDeadlineElapsedDoesNotThrowObjectDisposed` (originally: a cached lambda's captured `DeadlineGuard` could point at an already-disposed `CancellationTokenSource`; `DeadlineGuard.Check()` swallowed that rather than let a stale deadline throw `ObjectDisposedException` — **superseded same day**: swallowing tolerated stale per-execution state outliving its owning execution, the same defect class DiVoid #7880 fixed for a cached lambda's `StepBudget` (`InvokeAsExecution` opening a fresh per-dispatch step budget rather than reusing the defining execution's, PR #21); the fix is now structural — `ScriptContext.Capture(context)` is the dedicated factory `LambdaToken.Execute` (`Tokens/LambdaToken.cs:53`) uses to build the context a `LambdaMethod` captures, and it deliberately does not carry `DeadlineGuard` forward, so `Invoke()`/`InvokeFrom()` on a cached lambda has no deadline at all — matching the pre-existing documented contract (`docs/pooscript-language-reference.md` §12) rather than an incidental side effect of a caught exception. The `try`/`catch` in `DeadlineGuard.Check()` is gone; the `ObjectDisposedException` path is unreachable, not caught). No text elsewhere in this table is rewritten — this note is the pointer.
