...
| A2 | `IScript` and `IExternalMethod` have no external implementers. In-repo, `IScript` is implemented only by the internal `Script` class; `IExternalMethod` only by `ExternalScriptMethod`. | Verified by grep across `Pooshit.Scripts/`, `Scripting.Tests/`, `ScriptExecutor/` |
...
|---|---|---|
| **1 — Primitive** | `ScriptLimits` (three nullable knobs + a shared no-limits instance); `StepBudget` (internal, interlocked); `ScriptContext` gains `Limits`, the budget reference and `Guard()`; derived ctor propagates all three. `ScriptStepLimitExceededException`, `ScriptTimeoutException` (both `: ScriptException`). | — |
